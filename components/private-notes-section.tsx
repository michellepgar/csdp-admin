...
} from "@/app/(app)/private-notes/actions";

import { PRIVATE_NOTES_HREF } from "@/lib/workspace-last-place";

/* The Private Notes tab of My Workspace: every note you wrote or that was
   shared with you -- search, the Priority Board, and the list. (It used to be
   its own page; /private-notes now forwards here.) */
export async function PrivateNotesSection({ q = "", highlightNote }: { q?: string; highlightNote?: string }) {
  ...
  return (
    <>
        ...
        <form method="get" action="/my-workspace" className="flex max-w-3xl flex-wrap items-center gap-2" role="search">
          ...
          {searching && (
            <Link href={PRIVATE_NOTES_HREF} prefetch={false} className="text-sm text-primary underline underline-offset-2">
              Clear search
            </Link>
          )}
        </form>
