...
--
-- The little "!" flags in Michelle's original paste were dropped from
-- names here (assumed to be her own highlight/flag, not part of the
-- name) -- see the chat summary for exactly which ones had a flag.
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    front_desk = 'Kat Porzelt', front_desk_email = 'kathleenporzelt@bpsma.org',
    nurse_name = E'Amanda Phillips\nLaurie Abban', nurse_email = E'amandaphillips@bpsma.org\nlaurieabban@bpsma.org'
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Casey Dimitri', nurse_email = 'caseydimitri@bpsma.org'
...
END $$;
-- Arnone note (not written to any column -- see chat): ask if there's
-- another nurse also covering the front desk, since Ayla moved to a
-- new school.
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Lynn Adams', nurse_email = 'lynnadams@bpsma.org'
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    front_desk = 'Vera Morris', front_desk_email = 'veramorris@bpsma.org',
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    front_desk = 'Tina Kusick', front_desk_email = 'TINAMARIEKUSICK@bpsma.org',
    nurse_name = 'Doreen Allen Kellum', nurse_email = 'doreenallenkellum@bpsma.org'
...
END $$;
-- Downey note (not written -- see chat): ask Tina Kusick for her email
-- address; two new nurses, Jacinta Tribou and Kendra DaCosta, no
-- emails given yet.
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    front_desk = 'Grace',
    nurse_name = 'Diane Sloane', nurse_email = 'dianesloane@bpsma.org'
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Regina Papp, Bs, M.Ed., Rn', nurse_email = 'reginapapp@bpsma.org'
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Ann Nardelli', nurse_email = 'ANNNARDELLI@bpsma.org'
...
-- save a wrong email. Also: no contact details listed for the new
-- Asst Principal on the school website, per Michelle's own note.

DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    principal = 'Meghann Bottome',
    nurse_name = 'Astride Jeune', nurse_email = 'ASTRIDEJEUNE@bpsma.org'
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Kimberly Baldi', nurse_email = 'kimberlybaldi@bpsma.org'
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Kerry Rodrick', nurse_email = 'KERRYRODRICK@bpsma.org'
...
END $$;
-- Plouffe: Michelle's note says the school website lists Sean Ahern
-- as Principal and Jaime Estee as "Associate Principal" -- the sheet
-- itself put Jaime under Front Desk, kept as the sheet has it since
-- that's what was given, but worth checking whether Jaime should
-- actually be Asst Principal instead.
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    front_desk = 'Tanya Lincoln', front_desk_email = 'tanyalincoln@bpsma.org',
    nurse_name = 'Lisa Jezaed', nurse_email = 'lisajezard@bpsma.org'
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Darcie Edwards', nurse_email = 'DARCIEEDWARDS@bpsma.org'
...
END $$;
-- Champion note (not written -- no column it clearly belongs to): a
-- "Jennifer Buckley JENNIFERBUCKLEY@bpsma.org" was listed under the
-- Nurse Email column but doesn't look like a nurse -- ask Michelle
-- who Jennifer Buckley is / what position, then decide where it goes.
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Allison Smith', nurse_email = 'allisonsmith@bpsma.org'
...
END $$;
-- Promise note (not written): "Stephanie Spillane - Academic
-- Associate Principal" -- unclear if this is a second Asst Principal
-- (that field only holds one name) or belongs somewhere else; ask
-- Michelle.
...
DO $$ DECLARE r int; BEGIN
  UPDATE contact_rows cr SET
    ...
    nurse_name = 'Rachael Nazon', nurse_email = 'RACHELNAZON@bpsma.org'
...
-- right:
SELECT cg.name AS "group", cr.school, cr.principal, cr.asst_principal, cr.front_desk, cr.nurse_name
