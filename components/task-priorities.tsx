...
/* Boss-only "what should someone work on next" list, shown beside
   ...
   priorities nobody has grabbed yet show here. One assigned to a person
   goes straight onto their Planned Work (and they're notified); one
   anyone grabs (Today or Planned work) moves there too. */
            ...
            {shared.map((item, index) => editingId === item.id ? (
              ...
              <li key={item.id} className="space-y-2 rounded-lg border border-l-4 border-red-500/25 border-l-red-600 bg-card px-3 py-2.5 text-sm shadow-sm transition-shadow hover:shadow-md">
                ...
                <div className="flex flex-wrap items-center gap-1.5 border-t border-red-500/15 pt-2">
                  ...
                  <form action={claimPriorityPlanItem}><input type="hidden" name="id" value={item.id} /><SubmitButton variant="outline" size="xs" pendingLabel="…" title="Add to my Planned Work"><CalendarClock className="h-3 w-3" /> Planned work</SubmitButton></form>
                </div>
              </li>
            ))}
          </ul>
